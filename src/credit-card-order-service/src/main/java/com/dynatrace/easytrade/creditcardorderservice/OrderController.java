...
public class OrderController {
    ...
    public ResponseEntity<StandardResponse> getStatusHistory(@PathVariable Integer accountId) {
        ...
        try (Connection conn = dbHelper.getConnection()) {
            ...
            if (client.getBooleanValue("large_payload", false)){
                logger.warn("Simulating large payload.");
                StringBuilder largePayload = new StringBuilder();
                for (int i = 0; i < 1000000; i++) { // Generate a 1MB+ payload
                    largePayload.append("A");
                }

                return buildResponseEntity(HttpStatus.OK, "large Payload", largePayload.toString());
            }
    ...
    public ResponseEntity<?> getLatestStatus(@PathVariable Integer accountId) {
        ...
        try (Connection conn = dbHelper.getConnection()) {
            ...
            if (client.getBooleanValue("large_payload", false)){
                logger.warn("Simulating large payload.");
                StringBuilder largePayload = new StringBuilder();
                for (int i = 0; i < 1000000; i++) { // Generate a large payload
                    largePayload.append("A");
                }
                return buildResponseEntity(HttpStatus.OK, "large Payload", largePayload.toString());
            ...
            if (client.getBooleanValue("undefined_variable_error", false)) {
                ...
                return ResponseEntity
                        ...
                        .body("<script>console.log(nonExistentVariable);</script>");

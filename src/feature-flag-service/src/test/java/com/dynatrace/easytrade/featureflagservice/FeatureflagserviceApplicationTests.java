...
class FeatureflagserviceApplicationTests {
    ...
    // Updated mockFlags to use Map<String, List<Flag>> (grouped structure)
    private final Map<String, List<Flag>> mockFlags = Map.of(
            "Feature Management", List.of(
                    new Flag("FLAG_1", true, "TEST FLAG 1", "This is a test flag 1", true, "config", "Feature Management"),
                    new Flag("FLAG_3", true, "TEST FLAG 3", "This is a test flag 3", false, "config", "Feature Management")
            ),
            "Error Handling", List.of(
                    new Flag("FLAG_2", false, "TEST FLAG 2", "This is a test flag 2", true, "problem_pattern", "Error Handling")
            )
    );

    @BeforeEach
    void setUp() {
        flagService = new FlagService(mockFlags); // Pass grouped flags
    ...
    void shouldGetAllFlags() throws Exception {
        ...
        assertEquals(
                "{\"results\":["
                        + "{\"id\":\"FLAG_3\",\"enabled\":true,\"name\":\"TEST FLAG 3\",\"description\":\"This is a test flag 3\",\"isModifiable\":false,\"tag\":\"config\",\"group\":\"Feature Management\"},"
                        + "{\"id\":\"FLAG_1\",\"enabled\":true,\"name\":\"TEST FLAG 1\",\"description\":\"This is a test flag 1\",\"isModifiable\":true,\"tag\":\"config\",\"group\":\"Feature Management\"},"
                        + "{\"id\":\"FLAG_2\",\"enabled\":false,\"name\":\"TEST FLAG 2\",\"description\":\"This is a test flag 2\",\"isModifiable\":true,\"tag\":\"problem_pattern\",\"group\":\"Error Handling\"}"
                        + "]}",
    ...
    void shouldGetEmptyListForNonExistentTag() throws Exception {
        ...
        assertEquals("{\"results\":[]}", mvcResult.getResponse().getContentAsString());
    }
    ...
    void shouldUpdateFlag() throws Exception {
        ...
        assertEquals(
                "{\"id\":\"FLAG_1\",\"enabled\":false,\"name\":\"TEST FLAG 1\",\"description\":\"This is a test flag 1\",\"isModifiable\":true,\"tag\":\"config\",\"group\":\"Feature Management\"}",
                mvcResult.getResponse().getContentAsString());
    }
